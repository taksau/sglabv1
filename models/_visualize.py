...
def draw_box(draw, boxx, cls_ind, text_str):
    ...
    if '-GT' in text_str:
        ...
        color = (169, 209, 142, 50)

    # color = tuple([int(x) for x in cmap(cls_ind)])

    # draw the fucking box
    draw.line([(box[0], box[1]), (box[2], box[1])], fill=color, width=4)
    draw.line([(box[2], box[1]), (box[2], box[3])], fill=color, width=4)
    draw.line([(box[2], box[3]), (box[0], box[3])], fill=color, width=4)
    draw.line([(box[0], box[3]), (box[0], box[1])], fill=color, width=4)
    ...
    draw.text((x1text, y1text), text_str, fill=color, font=font)
    return draw
...
def val_batch(batch_num, b, evaluator, thrs=(20, 50, 100)):
    ...
    containname = os.path.join(conf.save_dir, conf.mode+'-qualitative-top'+str(20))
    ...
    with open(os.path.join(pathname, 'shit.txt'), 'w') as f:
        pred_objs = ' ,'.join([train.ind_to_classes[i] for i in pred_entry['pred_classes']])
        gt_objs = ' ,'.join([train.ind_to_classes[i] for i in gt_entry['gt_classes']])
        f.write('pred objs:\n' + pred_objs + '\n')
        f.write('gt objs:\n' + gt_objs + '\n')
        f.write('good:\n')
