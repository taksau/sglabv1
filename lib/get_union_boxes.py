...
def geo_layout_encode(boxes, rel_inds):
    s_boxes = boxes[rel_inds[:, 1]]
    o_boxes = boxes[rel_inds[:, 2]]
    x_s, x_o, y_s, y_o = s_boxes[:, 0], o_boxes[:, 0], s_boxes[:, 3], o_boxes[:, 3]
    w_s, w_o = s_boxes[:, 2] - s_boxes[:, 0], o_boxes[:, 2] - o_boxes[:, 0]
    h_s, h_o = s_boxes[:, 3] - s_boxes[:, 1], o_boxes[:, 3] - o_boxes[:, 1]

    geo_code = torch.stack(((x_o-x_s)/w_s, (y_o-y_s)/h_s, torch.log(w_o/w_s), torch.log(h_o/h_s)), -1)
