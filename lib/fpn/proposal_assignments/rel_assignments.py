...
from config import BG_THRESH_HI, BG_THRESH_LO, REL_FG_FRACTION, RELS_PER_IMG_REFINE, RELS_PER_IMG
...
@to_variable
def rel_assignments_det(im_inds, rpn_rois, roi_gtlabels, gt_boxes, gt_classes, gt_rels, image_offset,
                    fg_thresh=0.5, num_sample_per_gt=4, filter_non_overlap=True):
    """
    Assign object detection proposals to ground-truth targets. Produces proposal
    classification labels and bounding-box regression targets.
    ...
    for im_ind in range(num_im):
        ...
        mask = (pred_boxlabels_i[:, None] == gt_classes_i[None, :]).astype(np.float32)
        ...
        all_rels_i[fg_inds_i, -1] = gt_rels_i[:, -1][min_ious_i.argmax(-1)[fg_inds_i]]
    ...
    if num_bg > 0:
        if bg_rels.shape[0] > num_bg:
            ...
            choice_ind = torch.randperm(bg_rels.shape[0])[:num_bg].numpy().astype(np.int64)
    ...
    rel_labels = torch.LongTensor(rel_labels).cuda(rpn_rois.get_device(), async=True)
