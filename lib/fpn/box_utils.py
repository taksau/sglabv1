...
def nms_cython(scores, boxes, pre_nms_topn=12000, post_nms_topn=2000, nms_thresh=0.7):
    
    if scores.size(0) > pre_nms_topn:
        vs, idx = torch.sort(scores, dim=0, descending=True)
        idx = idx[:pre_nms_topn]
    else:
        idx = torch.arange(0, boxes.size(0)).long().cuda()
    dets = torch.cat((boxes[idx], scores.unsqueeze(-1)[idx]), -1).cpu().numpy()
    keep = torch.from_numpy(cython_nms.nms(dets, nms_thresh)).long().cuda()
    idx = idx[keep]
    if idx.size(0) > post_nms_topn:
        vs, idx2 = torch.sort(scores[idx], dim=0, descending=True)
        idx2 = idx2[:post_nms_topn]
        idx = idx[idx2]
    return idx

def co_nms(p_dets, thresh):
    """Apply co NMS"""
    if p_dets.shape[0] == 0:
        return []
    return cython_nms.co_nms(p_dets, thresh)
