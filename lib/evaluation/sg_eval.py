...
class BasicSceneGraphEvaluator:
    def __init__(self, mode, multiple_preds=False):
        self.result_dict = {}
        self.mode = mode
        self.result_dict[self.mode + '_recall'] = {20: [], 50: [], 100: [], 200: []}
        self.result_dict[self.mode + '_pairrecall'] = {20: [], 50: [], 100: [], 200: []}
        self.result_dict[self.mode + '_pred_predicates'] = []
        self.result_dict[self.mode + '_gt_predicates'] = []
        self.result_dict[self.mode + '_objs_mAP'] = []
    ...
    def print_stats(self):
        ...
        for k, v in self.result_dict[self.mode + '_pairrecall'].items():
            print('R@%i: %f' % (k, np.mean(v)))
        pred_recall = np.sum(self.result_dict[self.mode + '_pred_predicates'], 0) / np.sum(self.result_dict[self.mode + '_gt_predicates'], 0)
        print('objs mAP: ',np.mean(self.result_dict[self.mode + '_objs_mAP']))
        # print('predicates_recall:\n', pred_recall)
        return pred_recall
...
def filter_out_ind(index, scores, max_ind):
    mask = (index < max_ind).prod(1).astype(np.bool)
    ret_index = index[mask]
    ret_scores = scores[mask]
    return ret_index, ret_scores
