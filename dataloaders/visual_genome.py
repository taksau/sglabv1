...
class VG(Dataset):
    def __init__(self, mode, roidb_file=VG_SGG_FN, dict_file=VG_SGG_DICT_FN,
        ...
        self.filenames, self.index_list = load_image_filenames(image_file)
        self.filenames = [self.filenames[i] for i in np.where(self.split_mask)[0]]
        self.index_list = [self.index_list[i] for i in np.where(self.split_mask)[0]]
    ...
    def __getitem__(self, index):
        ...
        image_index = self.index_list[index]
